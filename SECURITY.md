...
| ASD V-222430 | Execute without excessive permissions | Met | Non-root chiseled image. The serving identity has DML only in both databases and owns nothing (`aberaTech.Postgres/Sql/README.md`). The container app's identity holds one custom role on one VM (`DevBox/`) |
...
| ASD V-222614, V-222658 | Patches current, products supported | Met | Dependabot on every ecosystem with auto-merge for non-majors. The held-majors gate (`scripts/check-held-majors.sh`) finds a bump Dependabot cannot offer. .NET and Node on their newest GA majors (`scripts/check-newest-majors.sh`) |
...
| SP 800-53 AC-6, AC-6(10) | Least privilege. The serving identity cannot run privileged functions | Met | The serving identity is not the schema owner. `abera-migrator` owns both databases and every object in them, and no administrator holds membership in either role (2026-09-26). The deploy workflow's `migrate` job applies migrations as `abera-migrator`, a managed identity that only a `master` run can sign in as. The app has `SELECT, INSERT, UPDATE, DELETE` and cannot run `CREATE`, `ALTER`, `DROP`, `TRUNCATE` or temp tables. `DatabaseLeastPrivilegeTests` connects as the runtime role and is refused each one. Migrate-on-start is on in Development only (`DatabaseMigrationsTests`) |
