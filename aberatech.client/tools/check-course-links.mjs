...
const catalogueUrl = (code) =>
  `https://e-catalogue.jhu.edu/search/?P=${encodeURIComponent(code)}`;
...
for (const code of codes) {
  ...
  if (!titleMatches) {
    failures++;
    console.error(
      `FAIL ${code}: status ${status}, catalogue says ${catalogueTitle ?? "nothing"}, we say ${expected}`,
    );
  }
  results[code] = row;
...
const summary = {
  ...
  total: codes.length,
  resolvedWithMatchingTitle: codes.filter((c) => results[c].titleMatches)
    .length,
  courses: results,
};
...
if (write) {
  writeFileSync(
    join(dataDir, "link-check.json"),
    `${JSON.stringify(summary, null, 2)}\n`,
  );
}

console.log(
  `${summary.resolvedWithMatchingTitle} of ${summary.total} course links resolve with a matching title`,
);
