...
export default function AvailabilityEditor({ enabled }: { enabled: boolean }) {
  const { week, loading, error, saved, setWeek, save } =
    useAvailability(enabled);

  if (!enabled) return null;
  if (loading)
    return <CircularProgress size={24} aria-label="Loading your hours" />;
  if (!week) return error ? <Alert severity="error">{error}</Alert> : null;

  const update = (day: number, change: Partial<AvailabilityDay>) =>
    setWeek({
      ...week,
      days: week.days.map((d) => (d.day === day ? { ...d, ...change } : d)),
    });
        ...
        <Stack spacing={2}>
          <Box>
            ...
            <Typography variant="body2" sx={{ color: "text.secondary" }}>
              When to offer times at all. Anything on your Google calendar is
              taken out of these hours automatically.
            </Typography>
          </Box>
          ...
          <Box>
            ...
            <Autocomplete
              ...
              value={week.zoneId}
              onChange={(_, value) =>
                value ? setWeek({ ...week, zoneId: value }) : undefined
              }
              autoHighlight
              disableClearable
              size="small"
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  label="Time zone"
                  helperText="The hours below are read in this zone."
                />
              )}
            />
            ...
            {week.days.map((day) => (
              ...
              >
                <Switch
                  checked={day.active}
                  onChange={(event) =>
                    update(day.day, { active: event.target.checked })
                  }
                ...
                <TextField
                  ...
                  disabled={!day.active}
                  onChange={(event) =>
                    update(day.day, { startsAt: event.target.value })
                  }
                  sx={{ width: 128 }}
                />
                <Typography variant="body2" sx={{ color: "text.secondary" }}>
                ...
                <TextField
                  ...
                  disabled={!day.active}
                  onChange={(event) =>
                    update(day.day, { endsAt: event.target.value })
                  }
                  sx={{ width: 128 }}
                />
              </Box>
