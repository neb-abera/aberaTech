...
import { addMonths, longDayLabel, monthGrid, monthLabel, monthOf } from '../month';
...
describe('monthGrid', () => {
  ...
  it('starts with no blanks when the first is a Sunday', () => {
    ...
    expect(monthGrid(2027, 8)[0]).toEqual({ date: '2027-08-01', dayOfMonth: 1 });
  });

  it('has the right number of days for each month', () => {
    const days = (year: number, month: number) => monthGrid(year, month).filter((cell) => cell.date).length;
