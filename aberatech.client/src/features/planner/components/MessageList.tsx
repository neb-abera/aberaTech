    ...
    <Stack spacing={1}>
      {violations.slice(0, 6).map((v) => (
        <Alert severity="error" key={`${v.kind}-${v.code}-${v.detail}`}>
          {v.kind === "exclusion" ? (
            <>
              <b>{model.title(v.code)}</b> and <b>{model.title(v.detail)}</b>{" "}
      ...
      {violations.length === 0 && model.plan.courses().length > 0 && (
        <Alert severity="success">
          Every prerequisite is satisfied in sequence.
        </Alert>
      )}
        ...
        <Accordion disableGutters variant="outlined">
          <AccordionSummary expandIcon={<ExpandMore />}>
            <Typography variant="body2">
              <b>
                {auto.size} {plural(auto.size, "course")} pulled in
                automatically
              </b>{" "}
              because something you chose requires{" "}
              {auto.size > 1 ? "them" : "it"}. Marked <i>req</i>.
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            {[...auto].map((c) => (
        ...
        <Accordion disableGutters variant="outlined">
          <AccordionSummary expandIcon={<ExpandMore />}>
            <Typography variant="body2">
              ...
              in the plan. The catalog states these in prose rather than course
              numbers, so they are scheduled like any other prerequisite. Tick
              one under Background if you already have it and it disappears.
            </Typography>
          </AccordionSummary>
          <AccordionDetails>
            {prep.map((c) => (
