...
export default function NeedBar() {
  ...
  if (!needed.length) {
    return (
      <Alert severity="success" onClose={dismiss}>
        ...
        Every prerequisite is already in the plan. Drag it into any term
        outlined in green, or use the menu on the chip.
      </Alert>
    );
  }
  ...
  return (
    <Alert severity="info" onClose={dismiss}>
      <AlertTitle>
        {course.title} needs {needed.length} {plural(needed.length, "course")}{" "}
        first
      </AlertTitle>
      <Typography variant="body2" sx={{ mb: 1 }}>
        Outlined in green below. Add them yourself, or let the planner place
        them for you.
      </Typography>
