...
export default function GraduationPanel({
    ...
    return (
      <Paper variant="outlined" sx={{ p: 2 }}>
        ...
        <Typography variant="body2" sx={{ color: "text.secondary", mt: 0.5 }}>
          No course carrying graduate credit is placed yet, so the five year
          clock has not started. It begins with the first course you apply to
          the degree, not the first course you take.
        </Typography>
      </Paper>
    );
  }
  ...
  const deadlinePct =
    k.deadlineTerm === null
      ? null
      : Math.min(100, ((k.deadlineTerm - k.startTerm + 1) / span) * 100);
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" sx={{ color: "text.secondary", mt: 0.5 }}>
        {audit.counted.length} of {LIMITS.COURSES} courses applied to the
      ...
      <Box sx={{ position: "relative", mt: 2, mb: 3 }}>
        <Box
          sx={{ height: 16, borderRadius: 1, backgroundColor: "action.hover" }}
        />
        <Box
          sx={{
            position: "absolute",
        ...
        <Box component="tbody">
          {rows.map(([a, b, c]) => (
            <Box component="tr" key={a}>
              <Box
                component="td"
                sx={{ py: 0.4, pr: 1.5, whiteSpace: "nowrap" }}
              >
      ...
      <Section
        title={`The ten applied, ${audit.selection.automatic ? "chosen for you" : "chosen by you"}`}
      >
