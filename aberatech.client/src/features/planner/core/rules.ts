...
export function degreeAudit(plan: Plan, calendar: Calendar, opts: AuditOptions = {}): DegreeAudit {
  const cat = plan.catalog;
  const leave = clamp(opts.leaveMonths ?? 0, 0, LIMITS.MAX_LEAVE_MONTHS);
  const extension = clamp(opts.extensionMonths ?? 0, 0, LIMITS.MAX_EXTENSION_MONTHS);

  const picks = opts.picks && opts.picks.size ? opts.picks : null;
  ...
  plan.terms.forEach((t, i) => {
    for (const code of [...t].sort()) {
      ...
      termOf.set(code, i);
      if (!c.gradeable) {
        excluded.push({ code, why: 'carries no graduate credit, so it cannot be applied to the degree' });
        continue;
      }
  ...
  for (const c of candidates) {
    if (!countedSet.has(c.code)) excluded.push({ code: c.code, why: 'taken, but not applied to the degree' });
  }
  counted.sort((a, b) => (termOf.get(a) ?? 0) - (termOf.get(b) ?? 0) || a.localeCompare(b));

  const startTerm = counted.length ? (termOf.get(counted[0]) ?? null) : null;
  const finishTerm = counted.length ? (termOf.get(counted[counted.length - 1]) ?? null) : null;
  const startDate = startTerm === null ? null : calendar.startDate(startTerm);
  const deadline = startDate === null ? null : addMonths(Calendar.addYears(startDate, LIMITS.YEARS), leave + extension);
  const projected = finishTerm === null ? null : calendar.endDate(finishTerm);
  const slackMonths = deadline && projected ? Calendar.monthsBetween(projected, deadline) : null;
  ...
  const blockers: Blocker[] = rules.filter((r) => !r.met).map((r) => ({ id: r.id, detail: unmetDetail(r) }));
...
function unmetDetail(r: RuleRow): string {
  return r.inverse ? `${r.label}: you have ${r.have}` : `${r.label}: you have ${r.have} of ${r.need}`;
}

function buildLevers(slack: number | null, leave: number, extension: number): Lever[] {
  if (slack === null || slack >= 0) return [];
  const out: Lever[] = [
    {
      id: 'compress',
      title: 'Take more courses per term',
      detail: 'The cheapest fix. Moving from one course a term to two roughly halves the elapsed time.'
    }
  ];
