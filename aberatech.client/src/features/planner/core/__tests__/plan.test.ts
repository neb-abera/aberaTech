...
describe("placeWithPrerequisites", () => {
  ...
  test("satisfies an or group with the cheaper branch", () => {
    expect(planOf([[], []]).placeWithPrerequisites("E", 1).added).toEqual([
      "F",
    ]);
  });
  test("reuses an or branch that is already in the plan", () => {
    expect(
      planOf([["A"], ["B"], []]).placeWithPrerequisites("E", 2).added,
    ).toEqual([]);
  });
  ...
  test("the resulting plan has no prerequisite violations", () => {
    expect(
      planOf([[], [], []]).placeWithPrerequisites("D", 2).plan.violations(),
    ).toEqual([]);
  });
});
...
describe("autoSchedule", () => {
  ...
  test("leaves nothing behind when the set is closed", () => {
    expect(
      Plan.autoSchedule(TOY, ["A", "B", "C", "D", "E", "F"], 2).courses(),
    ).toHaveLength(6);
  });
...
describe("violations", () => {
  ...
  test("reports an unmet prerequisite", () => {
    expect(Plan.fromTerms(TOY, [["B"]]).violations()[0].kind).toBe(
      "prerequisite",
    );
  });
});
