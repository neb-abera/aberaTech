...
const cal = new Calendar({ startTerm: 'Spring', startYear: 2027, termsPerYear: 3 });
const audit = (terms: string[][], opts = {}) => degreeAudit(Plan.fromTerms(TOY, terms), cal, opts);
const rule = (rules: { id: string; met: boolean }[], id: string) => rules.find((r) => r.id === id)?.met;
...
describe('the five year clock', () => {
  ...
  test('a leave of absence is capped at two years', () => {
    ...
    expect(monthsOfClock(a.clock.startDate, a.clock.deadline)).toBe(60 + LIMITS.MAX_LEAVE_MONTHS);
  });
...
describe('projected completion against the deadline', () => {
  const cat: Catalog = { ...TOY };
  for (let i = 0; i < 10; i++) cat[`Y${i}`] = toyCourse(`Y${i}`, `Y${i}`, { level: i < 4 ? 7 : 6, areas: [] });
...
describe('the four degree rules', () => {
  ...
  test('at most three from outside the program', () => {
    expect(rule(mk(10, { sevens: 4, external: 3 }).rules, 'outside')).toBe(true);
    expect(rule(mk(10, { sevens: 4, external: 4 }).rules, 'outside')).toBe(false);
  });
  test('at least seven from within the program', () => {
    expect(rule(mk(10, { sevens: 4, external: 4 }).rules, 'inProgram')).toBe(false);
  });
...
describe('choosing which ten courses to apply', () => {
  ...
  for (let i = 0; i < 10; i++) cat[`S${i}`] = toyCourse(`S${i}`, `S${i}`, { areas: [] });
  for (let i = 0; i < 4; i++) cat[`S7${i}`] = toyCourse(`S7${i}`, `S7${i}`, { level: 7, areas: [] });
  ...
  test('the chosen ten satisfy every rule when a satisfying set exists', () => {
    ...
    expect(a.counted.filter((c) => cat[c].level >= 7).length).toBeGreaterThanOrEqual(4);
