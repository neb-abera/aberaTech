...
describe('catalogue links', () => {
  test('the address is the one the catalogue itself uses for a course', () => {
    expect(catalogueUrl('EN.525.614')).toBe('https://e-catalogue.jhu.edu/search/?P=EN.525.614');
  });
  ...
  test('the recorded catalogue title is the title we show, for every course', () => {
    ...
    const wrong = Object.entries(linkCheck.courses).filter(
      ([code, v]) => norm(v.catalogueTitle ?? '') !== norm(data.courses[code].title)
    );
...
describe('offerings links', () => {
  ...
  const confirmed: [string, string, string][] = [
    ['EN.525.728', 'Detection & Estimation Theory', 'https://ep.jhu.edu/courses/525728-detection-estimation-theory/'],
    ['EN.525.618', 'Antenna Systems', 'https://ep.jhu.edu/courses/525618-antenna-systems/'],
    [
      'EN.525.614',
  ...
  test('the course number keeps no separator, which is what the old bug got wrong', () => {
    ...
    expect(professionalsUrl('EN.525.001', 'A Course')).not.toContain('/525-001-');
  });

  test('runs of punctuation collapse rather than leaving empty segments', () => {
    expect(professionalsUrl('EN.525.001', 'A -- B,  C')).toBe('https://ep.jhu.edu/courses/525001-a-b-c/');
  });
});
...
describe('how links are presented', () => {
  ...
  test('an unverified link says so in its own note rather than relying on the flag', () => {
    const derived = courseLinks('EN.525.618', 'Antenna Systems').find((l) => !l.verified);
