...
const SIGNAL_CHAIN: Record<string, string[]> = {
  ...
  hardware: ['EN.525.642', 'EN.525.677', 'EN.525.612', 'EN.525.742', 'EN.525.743'],
  ...
  for (const t of tracks.all()) {
    describe(t.name, () => {
      ...
      test('every stage course appears in the flattened list and the other way round', () => {
        expect([...t.courses].sort()).toEqual(t.stages.flatMap((s) => s.courses).sort());
      });
      test('it is prerequisite closed: nothing has to be silently added', () => {
        const added = [...closure(data.courses, t.courses)].filter((c) => !t.courses.includes(c));
        expect(added, `closure would add ${added.map((c) => data.title(c)).join(', ')}`).toEqual([]);
      });
      ...
      test('the stages come out in order', () => {
        ...
        for (let i = 1; i < t.stages.length; i++) {
          const prevMax = Math.max(...t.stages[i - 1].courses.map((c) => p.get(c) ?? 0));
          const thisMax = Math.max(...t.stages[i].courses.map((c) => p.get(c) ?? 0));
          expect(thisMax, `stage "${t.stages[i].name}" finishes too early`).toBeGreaterThanOrEqual(prevMax);
        }
      });
      ...
      test('the applied ten finish inside the five year limit', () => {
        const a = degreeAudit(tracks.toPlan(data.courses, t.id, 2), cal, {});
        expect(a.clock.onTime, `over by ${-(a.clock.slackMonths ?? 0)} months`).toBe(true);
      });
      test(
        t.kind === 'degree'
        ...
        () => {
          if (t.kind === 'degree') expect(t.courses).toHaveLength(LIMITS.COURSES);
      ...
      test('it carries a goal and an honest tradeoff', () => {
        ...
        expect(t.tradeoff.length, 'every track must say what it costs you').toBeGreaterThan(40);
      });
    });
  }
});
