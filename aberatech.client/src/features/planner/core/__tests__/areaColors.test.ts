...
describe("area colours", () => {
  test("every group in the catalog has a colour assigned", () => {
    expect(
      Object.keys(data.areas).filter((a) => !AREA_NAMES.includes(a)),
    ).toEqual([]);
  });
  ...
  test("preparation has its own colour, distinct from every focus area", () => {
    ...
    const others = AREA_NAMES.filter((a) => a !== "Preparation").map((a) =>
      areaColor(a, "light"),
    );
    expect(others).not.toContain(prep);
  });
  test("an unknown group falls back rather than rendering with nothing", () => {
    expect(areaColor("Something Else", "light")).toBe(
      areaColor("Signal Processing", "light"),
    );
  });
  test("a course keys to the group the reader has turned on", () => {
    const areas = ["Signal Processing", "RF and Microwave Engineering"];
    expect(
      courseColor(areas, new Set(["RF and Microwave Engineering"]), "light"),
    ).toBe(areaColor("RF and Microwave Engineering", "light"));
    expect(courseColor(areas, new Set(["Signal Processing"]), "light")).toBe(
      areaColor("Signal Processing", "light"),
    );
  });
