...
describe("withBackground", () => {
  ...
  test("a preparation course never counts toward the degree", () => {
    expect(withBackground(BASE, BG, new Set())[prepId("bg_de")].gradeable).toBe(
      false,
    );
  });
  test("an unheld background item becomes a hard prerequisite", () => {
    expect(withBackground(BASE, BG, new Set()).A.groups).toEqual([
      [prepId("bg_de")],
    ]);
  });
  ...
  test("existing course prerequisites are preserved alongside", () => {
    expect(withBackground(BASE, BG, new Set()).B.groups).toEqual([
      ["A"],
      [prepId("bg_de")],
      [prepId("bg_la")],
    ]);
...
describe("planning with background", () => {
  ...
  test("missingFor names the preparation you still owe", () => {
    expect(
      missingFor(withBackground(BASE, BG, new Set()), "A", new Set()),
    ).toEqual([prepId("bg_de")]);
  });
...
describe("a composite background item", () => {
  ...
  test("is never scheduled, however little of it is held", () => {
    expect(
      withBackground(CAT, COMP, new Set())[prepId("bg_ee")],
    ).toBeUndefined();
  });
  ...
  test("opting in schedules the parts that are missing, and only those", () => {
    const cat = withBackground(
      CAT,
      COMP,
      new Set(["bg_calc", "bg_phys"]),
      new Set(["bg_ee"]),
    );
    const want = PARTS.filter((p) => p !== "bg_calc" && p !== "bg_phys").map(
      (p) => [prepId(p)],
    );
  ...
  test("holding every part satisfies it without ticking it", () => {
    ...
    expect(
      withBackground(CAT, COMP, held, new Set(["bg_ee"])).G.groups,
    ).toEqual([]);
  });
