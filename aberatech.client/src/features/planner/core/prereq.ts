...
export function unmetGroups(
  ...
  return groupsOf(cat, code).filter(
    (g) => !g.some((m) => placed.has(m) && (placed.get(m) ?? 0) < term),
  );
}
...
export function closureCost(
  ...
  for (const g of groupsOf(cat, code)) {
    ...
    if (opts.length)
      n += Math.min(
        ...opts.map((m) => closureCost(cat, m, new Set(seen), memo)),
      );
  }
  memo.set(code, n);
  return n;
...
export function latestTerm(
  ...
  for (const [dep, depTerm] of placed) {
    ...
    for (const g of groupsOf(cat, dep)) {
      ...
      const others = g.some(
        (m) => m !== code && placed.has(m) && (placed.get(m) ?? 0) < depTerm,
      );
